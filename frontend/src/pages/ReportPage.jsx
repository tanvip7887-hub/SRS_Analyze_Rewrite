...
const exportIssuesReport = (reqs, projectName) => {
  const ambiguous = reqs.filter(r => r.is_ambiguous).sort((a, b) => (b.ambiguity_score || 0) - (a.ambiguity_score || 0));

  let txt = `REQUIREMENT ISSUES REPORT (AMBIGUITY)\n`;
  txt += `Project: ${projectName}\n`;
  txt += `Generated: ${new Date().toLocaleString()}\n`;
  txt += `${'─'.repeat(70)}\n\n`;
  txt += `INSIGHTS SUMMARY\n`;
  txt += `Total Flagged Issues: ${ambiguous.length}\n`;
  txt += `Average Risk Score: ${ambiguous.length ? (ambiguous.reduce((s, r) => s + (r.ambiguity_score || 0), 0) / ambiguous.length * 100).toFixed(1) : 0}%\n\n`;
  txt += `${'─'.repeat(70)}\n\n`;
  txt += `FLAGGED REQUIREMENTS\n\n`;
  ...
  downloadBlob(blob, `${projectName}_issues_report_${timestamp()}.txt`);
};

// 3. Duplicate pairs report
const exportConflictsReport = (reqs, projectName) => {
  const dupMap = {};
  ...
  let txt = `REQUIREMENT CONFLICTS REPORT (DUPLICATES)\n`;
  txt += `Project: ${projectName}\n`;
  txt += `Generated: ${new Date().toLocaleString()}\n`;
  txt += `${'─'.repeat(70)}\n\n`;
  txt += `INSIGHTS SUMMARY\n`;
  txt += `Total Conflict Groups: ${Object.keys(dupMap).length}\n`;
  txt += `Total Redundant Requirements: ${reqs.filter(r => r.is_duplicate).length}\n\n`;
  txt += `${'─'.repeat(70)}\n\n`;
  txt += `CONFLICT GROUPS\n\n`;
  ...
  const blob = new Blob([txt], { type: 'text/plain' });
  downloadBlob(blob, `${projectName}_conflicts_report_${timestamp()}.txt`);
};
...
export default function ReportPage() {
  ...
  const EXPORTS = [
    ...
    },
    {
      key: 'issues',
      icon: AlertTriangle,
      title: 'Issues & Risk Report',
      description: 'Detailed analysis of all flagged requirements with ambiguity scores and AI-suggested rewrites.',
      badge: `${stats.ambig} issues`,
      color: '#f87171',
      bg: 'rgba(248,113,113,0.1)',
      fn: exportIssuesReport,
    },
    {
      key: 'conflicts',
      icon: Copy,
      title: 'Conflict Inventory',
      description: 'A catalog of all semantically similar requirement groups detected in the document.',
      badge: `${stats.dupGroups} groups`,
      color: '#fbbf24',
      bg: 'rgba(251,191,36,0.1)',
      fn: exportConflictsReport,
    },
    {
      key: 'csv',
        ...
        <div style={{ background: '#0d1018', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '18px', padding: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: '12px' }}>
            {[
              ...
              { val: stats.clean, label: 'Verified', color: '#4ade80' },
              { val: stats.ambig, label: 'Issues', color: '#f87171' },
              { val: stats.dups, label: 'Conflicts', color: '#fbbf24' },
